// Helper functions to handle rendering things to the screen so you can
//  focus on writing the JS logic.
// If you're interested, you can learn everything this file is doing by
//  reading through the comments and doing some Googling.
...
// Call this function each time you make a change and
  //  want that change to appear on the screen.
  //  Otherwise, your "code" will know that the change happened,
  //  but the screen won't know that it's supposed to update.

// INTERNAL: There are more elegant ways of doing this
  //  (right now we're wiping out the existing rendered gameBoard entirely
  //    and rendering the whole new one).
  ...
  //    and what the goal of the course is (writing JS logic/functional programming),
  //    and really not have to think about DOM or rendering stuff much at all.
...
var renderGameBoard = function(gameBoard) {
  ...
  //  First we find which is smaller, the height or width of the user's browser
  var browserSize = Math.min($(window).height(), $(window).width());

  $('.gameBoard').width(browserSize - 110);

  // then we leave some room around the edges (200 pixels),
  //  and divide by the number of squares to find how large
  //  the squares should be to fill that space perfectly.
  ...
  gameBoard.forEach(function(rowArr, rowIndex) {
    rowArr.forEach(function(squareObj, columnIndex) {
      ...
      // Here we are creating the HTML that will be rendered to the DOM
        //  for each square.
        // HTML and JS play nicely together;
          //  you can just create a string with most of the characters that you need,
          //  add in some variables dynamically, and
          //  then when you render this to the DOM, it will interpret everything
          //  to be HTML elements and display them correctly.

        // We're creating a <div>, which is just a default html container
          //  that we can do whatever we want with (similar to an object in JS).
        // We can then set "properties" on this html element.
          //  In this case, we're setting style properties to tell it
        ...
        // To keep track of which square this is
          //  (necessary for figuring out which square was clicked on later),
          //  we set a data "property" on each square as well.

        // Inside of each div, we can put whatever text we want!
          //  Or none at all- it doesn't care.
          //  So we put in the text from the object at that position, if one exists.

        // OPTIONAL: You can change what gets rendered for each square.
          //  Want to display the name differently?
          //  Feel free to modify the code below to do what you want!
...
// NOTE: You have to uncomment these lines to make program invoke
  //  the clickHandler function you're building out in yourOwnGame.js.
...
//initialPosition should be an array with two numbers in it.
  // those numbers should specify the 0-indexed row and column
  //   you want this piece to start at.
  // example: [1,3] would put the piece
  //   on the second row (remember we're 0-indexed)
  //   in the 4th column.
...
var makePiece = function(gameBoard, initialPosition, pieceType, playerBelongsTo) {
  ...
  // default player to Player1 if no player name is passed in,
  //  then define a unique name for this gamePiece
